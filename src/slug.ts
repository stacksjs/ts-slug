import type { SlugFunction, SlugOptions } from './types'
import { getWholeCharAndI } from './utils'

/* global btoa */
let base64: (input: string) => string | Promise<string>

if (typeof window !== 'undefined') {
  // @ts-expect-error - window.btoa is not present in React Native
  if (window.btoa) {
...
// Forward declaration of slug to avoid "used before defined" error
const slug = slugFunc as SlugFunction

function slugFunc(string: string, opts?: SlugOptions | string): string {
  // Debug mode for tests - returns original string and codepoints
  if (opts && typeof opts === 'object' && opts.debug === true) {
    // For debugging, just return the original string and convert space-separated words to arrays of code points
    const words = string.split(' ')
    const codePoints = words.map((word: string) => {
      return Array.from(word).map((char: string) => char.codePointAt(0))
    })
    return `STRING: ${string}\nCODEPOINTS: ${JSON.stringify(codePoints)}`
  }

  let result = slugify(string, opts)
  const fallback = opts && typeof opts === 'object' && opts.fallback !== undefined ? opts.fallback : slug.defaults.fallback
  ...
  if (fallback === true && result === '') {
    ...
    // Handle possible Promise returned by base64
    const encoded = base64(input)
    if (typeof encoded === 'string') {
      result = slugify(encoded, opts)
    }
  }
...
export function slugify(string: string, opts?: SlugOptions | string): string {
  if (typeof string !== 'string') {
    throw new TypeError(`slug() requires a string argument, received ${typeof string}`)
  }

  let options: SlugOptions
  if (typeof opts === 'string') {
    options = { replacement: opts }
  }
  else {
  ...
  const defaults = slug.defaults.modes[options.mode]
  const keys = ['replacement', 'multicharmap', 'charmap', 'remove', 'lower', 'trim']

  for (let key, i = 0, l = keys.length; i < l; i++) {
    key = keys[i]
    options[key as keyof SlugOptions] = (key in options)
      ? options[key as keyof SlugOptions]
      : (defaults[key as keyof SlugOptions] as any)
  }

  const localeMap = options.locale ? locales[options.locale] || defaultLocale : defaultLocale

  let lengths: number[] = []
  if (options.multicharmap) {
    for (const key in options.multicharmap) {
      if (!Object.prototype.hasOwnProperty.call(options.multicharmap, key)) {
        continue
      }
  ...
  const disallowedChars = options.mode === 'rfc3986' ? /[^\w\s\-.~]/ : /[^A-Z0-9\s]/i
  ...
  for (let char, i = 0, l = string.length; i < l; i++) {
    ...
    for (let j = 0; j < lengths.length; j++) {
      ...
      if (options.multicharmap && options.multicharmap[str]) {
        i += len - 1
        char = options.multicharmap[str]
    ...
    if (!matchedMultichar) {
      ...
      else if (options.charmap && options.charmap[char]) {
        char = options.charmap[char].replace(options.replacement as string, ' ')
      }
      else if (options.replacement && char.includes(options.replacement)) {
        // preserve the replacement character in case it is excluded by disallowedChars
        char = char.replace(options.replacement, ' ')
      }
      else {
  ...
  result = result.replace(/\s+/g, options.replacement as string) // convert spaces
  if (options.lower) {
